...
- **Save (or Delete) fails, or edits don't seem to stick after a restart:** the add-on persists its config to Home Assistant's Supervisor, which needs a specific permission role. If Save or Delete returns a warning about persisting (shown in amber, not red — the change is still live, just not durable yet), check the add-on's log — a permission (403) error there means the role requested in this add-on's manifest wasn't sufficient on your Supervisor version; the edit still works for previewing/rendering, but won't survive an add-on restart until this is resolved. See `config.yaml`'s `hassio_role` comment for how to test a narrower role yourself.
- **Panel never shows a fresh image:** check, in order: (1) is `dashboard_url` reachable at all from a browser on the same network as the add-on — try loading it directly; (2) does it skip the login page (see Trusted Networks above) — a login page screenshotted instead of your dashboard is a common silent failure; (3) does Panel width/height actually match what your firmware was compiled with (see "Panel size and other hardware" above) — a mismatch produces a file the firmware can't correctly decode, not an error the add-on can see.
- **Preview or refresh returns an error:** the message should tell you which of the above it is (unreachable URL vs. a render timeout vs. a bad panel size). If it's an unreachable URL, double check the URL loads in an ordinary browser first before assuming this add-on is at fault.
